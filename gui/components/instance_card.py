...
class CustomOCRRegionDialog(tk.Toplevel):
    def __init__(self, parent, screenshot=None):
        ...
        # Create variables for coordinate inputs
    ...
    def _save_region_to_desktop(self, x, y, w, h):
        """Save the selected region as an image to the desktop"""
    ...
    def _update_preview(self):
        ...
        try:
            ...
            if self.screenshot and isinstance(self.screenshot, Image.Image):
                ...
                self.preview_canvas.create_line(px2, py2, px2 - marker_size, fill="#00ff88", width=2)
                
                # Add coordinates text
                self.preview_canvas.create_text(
                    10,  # Fixed position in bottom-left
                    self.screenshot.height - 10,
                    text=f"({int(x*100)}%, {int(y*100)}%) - ({int((x+w)*100)}%, {int((y+h)*100)}%)",
                    fill="#8b949e",
                    font=("Segoe UI", 9),
                    anchor="sw"
                )
